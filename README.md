...
Tuoming Agent（旅数智析）是一个本地优先、隐私保护的 AI 数据分析工作台。CSV 和 Excel 在本机完成文件安全检查、敏感字段识别、稳定脱敏、清洗、白名单分析和 Plotly 可视化；外部模型只接收经过 DLP 检查的最小化脱敏上下文。
...
模型返回受 Pydantic 校验的结构化 AnalysisPlan
        ↓
用户确认后，本机按顺序执行清洗、分析和聚合白名单操作
        ↓
按需自动进入本地 Plotly BI 仪表盘；可继续追问、筛选、合并或授权导出
```
...
- DeepSeek 等云端模型可在同一份结构化计划中组合数据清洗、计算分析和 BI 看板意图；程序不会开放任意 Python/SQL，也不会把本地图表数据上传给模型。
- 执行失败时只保存、显示并回传固定安全错误码与提示，不把底层异常、文件路径、SQL 或数据片段放入修订上下文。
...
- **分析**：明确选择主数据制品，先预览中文结构化清洗、分析和看板计划；确认后才在本地白名单执行，并通过确定性质量门。可在“对话记录管理”中二次确认删除当前对话、关联计划和执行历史，上传数据与结果制品不会被删除。
- **仪表盘**：优先读取最新分析结果，识别用户对趋势、排名、占比、构成和指标关系的要求；本地生成最多 4 个 KPI，并按意图组合 Plotly 折线图、面积图、柱状图、饼图和散点图。未指定图形时会根据数值、时间和分类字段自动选图，避免重复图表；同时提供 100 行脱敏明细，可悬停查看、缩放和下载 PNG。
...
- Plotly 图表由桌面应用本地渲染，不连接 Plotly Chart Studio，也不会上传图表数据。
- KPI 会分批扫描本地 Parquet 制品；趋势、分类和关系查询复用白名单 DuckDB 计划，不接受用户或模型生成的 SQL。模型只声明图表类型与字段，程序会再次按本地制品结构校验。
- 单张图最多 200 个数据点，趋势最多 4 条序列，分类图默认显示前 15 项，明细最多 100 行，避免大文件在浏览器中一次性展开。
- 仪表盘始终读取脱敏制品，不能在页面中请求还原敏感值；需要还原时仍须进入“结果”页面并走原有授权导出流程。
...
项目在源码配置和 Windows 便携版启动参数中都强制使用 Streamlit `viewer` 工具栏模式，避免浏览器中 `Ctrl+C` 复制误触“清除缓存”。更新后需要完全退出旧程序并重新启动。
