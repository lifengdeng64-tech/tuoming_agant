...
- **仪表盘**：优先读取最新分析结果，自动识别数值、时间和分类字段；生成最多 4 个 KPI、Plotly 趋势图、分类对比图和 100 行脱敏明细。可切换数据制品、指标、聚合方式与维度，并在图表中悬停查看、缩放和下载 PNG。
...
- KPI 会分批扫描本地 Parquet 制品；趋势和分类查询复用白名单 DuckDB 计划，不接受用户或模型生成的 SQL。
- 单张图最多 200 个数据点，趋势最多 4 条序列，分类图默认显示前 20 项，明细最多 100 行，避免大文件在浏览器中一次性展开。
